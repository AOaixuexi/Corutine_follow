...
/**
 * 1. hook_init() 函数会在程序启动时调用，用于初始化所有的 hook 函数
 * 2. 通过 dlsym 函数获取原始函数的地址，然后将其赋值给 hook 函数
 * 3. 通过宏定义 HOOK_FUN 定义了所有需要 hook 的函数，然后通过 XX 宏展开，生成对应的函数声明和定义
 * 4. 在 hook 函数中，首先判断当前线程是否开启了 hook 功能，如果没有，则直接调用原始函数
 * 5. 如果开启了 hook 功能，则获取 fd 对应的 FdCtx，如果没有，则直接调用原始函数
 * 6. 如果 fd 对应的 FdCtx 已经关闭，则直接返回 EBADF 错误
 * 7. 如果 fd 不是 socket 或者用户设置了非阻塞，则直接调用原始函数
 * 8. 如果 fd 是 socket 并且用户没有设置非阻塞，则调用 do_io 函数
 * 9. do_io 函数会根据当前线程是否开启了 hook 功能，以及 fd 对应的 FdCtx 是否存在，是否关闭，是否是 socket，是否设置了非阻塞等条件，来决定是否需要 hook
 * 10. do_io 函数会根据 fd 对应的 FdCtx 是否设置了超时时间，来决定是否需要添加定时器
 * 11. 如果需要添加定时器，则添加一个条件定时器，如果超时，则取消事件，否则调用 yield 函数让出当前线程
 * 12. 当事件就绪时，会调用 yield 函数唤醒当前线程，然后根据定时器是否超时，来决定是否返回 ETIMEDOUT 错误
 */
...
// socket⽤于创建套接字，需要在拿到fd后将其添加到FdManager
...
/**
 * 在 fcntl 函数的 hook 实现中，特别处理了 F_SETFL 和 F_GETFL 命令，以确保用户未显式设置的 NONBLOCK 模式不会暴露给用户
 */
...
/**
 * 在 setsockopt 函数的 hook 实现中，特别处理了 SO_RCVTIMEO 和 SO_SNDTIMEO 选项，以确保超时设置能够正确反映到 FdCtx 中
 */
